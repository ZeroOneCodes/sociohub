...
const validateMediaFiles = (files, platforms) => {
  ...
  const platformLimits = {
    ...
    linkedin: {
      ...
      maxFiles: 1 // LinkedIn only allows 1 media file per post
    }
  };
  ...
  const hasVideo = filesArray.some(file => file.mimetype && file.mimetype.startsWith('video/'));
  ...
  filesArray.forEach((file, index) => {
    ...
    if (platforms.twitter) {
      if (index >= platformLimits.twitter.maxFiles) {
        fileErrors.push(`Twitter only allows ${platformLimits.twitter.maxFiles} files per post`);
      } else {
        if (file.mimetype && file.mimetype.startsWith('image/')) {
          ...
          if (file.size > platformLimits.twitter.image.maxSize) {
            fileErrors.push(`Twitter image exceeds ${platformLimits.twitter.image.maxSize/1024/1024}MB limit`);
          }
          ...
          if (file.size > platformLimits.twitter.video.maxSize) {
            fileErrors.push(`Twitter video exceeds ${platformLimits.twitter.video.maxSize/1024/1024}MB limit`);
          }
        } else {
          fileErrors.push(`Twitter doesn't support ${file.mimetype || 'unknown'} files`);
        }
      }
    }
    ...
    if (platforms.linkedin) {
      if (index >= platformLimits.linkedin.maxFiles) {
        fileErrors.push(`LinkedIn only allows ${platformLimits.linkedin.maxFiles} file per post`);
      } else {
        if (file.mimetype && file.mimetype.startsWith('image/')) {
          ...
          if (file.size > platformLimits.linkedin.image.maxSize) {
            fileErrors.push(`LinkedIn image exceeds ${platformLimits.linkedin.image.maxSize/1024/1024}MB limit`);
          }
        } else if (file.mimetype && file.mimetype.startsWith('video/')) {
          ...
          if (file.size > platformLimits.linkedin.video.maxSize) {
            fileErrors.push(`LinkedIn video exceeds ${platformLimits.linkedin.video.maxSize/1024/1024}MB limit`);
          }
        } else {
          fileErrors.push(`LinkedIn doesn't support ${file.mimetype || 'unknown'} files`);
        }
      }
    }
...
if (mediaFiles.length > 0) {
  ...
  if (postToTwitter === "true") {
    const twitterVideos = mediaFiles.filter(file => 
